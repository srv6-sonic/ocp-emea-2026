...
LEAVES=(01T0 02T0)
HOSTS=(NIC-A NIC-B)
SONIC_IMAGE="cscarpit/srv6-sonic-vs:0.0.6"
SONIC_SERVERS=2
...
deploy_topology() {
    ...
    for cname in "${SPINES[@]}" "${LEAVES[@]}"; do
        ...
        # Match topotest-like FRR bring-up for SRv6 GRT.
        docker exec vs-$rname bash -lc "
            supervisorctl status bgpd >/dev/null 2>&1 || true
            supervisorctl status bgpd | grep -q RUNNING || supervisorctl start bgpd
            # Wait for bgpd socket to be ready before loading config
            sleep 20
            ip link show sr0 >/dev/null 2>&1 || ip link add sr0 type dummy
            ip link set sr0 up
            sysctl -w net.vrf.strict_mode=1 >/dev/null
            ip link show vrfdefault >/dev/null 2>&1 || ip link add vrfdefault type vrf table 254
            ip link set vrfdefault up
            vtysh -f /etc/frr/frr.conf
        " > /dev/null \
            || { echo "[ERROR] Failed FRR bring-up sequence in vs-$rname"; exit 1; }
    done
